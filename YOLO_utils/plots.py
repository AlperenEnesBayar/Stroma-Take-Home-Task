...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [0, 0, 0], thickness=tf + 1, lineType=cv2.LINE_AA)
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)


def plot_one_box_track(im0, x, y, id):
    # Plots one bounding box on image img
    tl = 1 or round(0.002 * (im0.shape[0] + im0.shape[1]) / 2) + 1  # line/font thickness
    cv2.putText(im0, str(id), org=(int(x + 2), int(y - 2)), fontFace=0, fontScale=0.5, color=[225, 255, 255], thickness=tl, lineType=cv2.LINE_AA)
